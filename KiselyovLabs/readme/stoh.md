...
В зависимости от _шага 3_ алгоритм дает как точный, так и неточный ответ (если,
например, выборки невозможно разделить прямой).

<u>Замечание</u>. Условие остановки на _шаге 3_ слишком мягкое для некоторых алгоритмов.
В результате, некоторые алгоритмы останавливаются, получив далеко не оптимальный
результат. Такая преждевременная остановка может получиться, когда алгоритм выбирает
похожие
![](http://latex.codecogs.com/svg.latex?x_i)
2 шага подряд. Чтобы избежать таких случаев немного изменим правило остановки:
_остановим алгоритм, если условие_
![](http://latex.codecogs.com/svg.latex?%5Cleft%28%7CQ_%7Bprev%7D%20-%20Q%7C%20%3C%20%5Cfrac%7B1%7D%7B100%7D%5Cright%29)
_выполняется 10 итераций подряд_.
